    ...
    const CheckAreaReport = () => {
      ...
      const handleSubmitReport = async () => {
        ...
        try {
          ...
          console.log("Supabase Public URL to be saved in DB:", photoPublicUrl);

          // 2. Save report to database with Supabase Storage URL
          console.log("Attempting to insert report into database with Supabase Storage URL:", photoPublicUrl);
          const { error: insertError } = await supabase
            .from('check_area_reports')
            .insert({
              user_id: user.id,
              location_id: locationId,
              photo_url: photoPublicUrl, // Save the Supabase Storage URL
            });
